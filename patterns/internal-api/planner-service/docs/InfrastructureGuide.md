...
The infrastructure is organized into one main AWS CDK stack:
...
| --------------- | -------------------------------- | ---------------------------------------------------------------- |
| Lambda Function | `{app-name}-daily-planner-{env}` | Daily planner handler (invokes Task Service list-tasks function) |
| API Gateway     | `{app-name}-api-{env}`           | REST API for Lambda functions with /planner/daily endpoint       |
...
| `DailyPlannerFunctionArn` | `{app-name}-daily-planner-function-arn-{env}` | Daily Planner Lambda function ARN |
