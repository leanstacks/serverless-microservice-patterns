...
export const handler = async (event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> => {
  ...
  logger.debug({ event, context }, '[CreateTaskHandler] handler - APIGatewayProxyEvent');
  ...
  try {
    ...
    if (error instanceof ZodError) {
      ...
      logger.warn({ error, validationMessages: errorMessages }, '[CreateTaskHandler] < handler - validation error');
