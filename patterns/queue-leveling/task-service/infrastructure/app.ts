...
// Get the Notification Queue ARN from the notification-service CloudFormation output
const notificationQueueArn = cdk.Fn.importValue(
  `smp-queue-leveling-notification-service-notification-queue-arn-${config.CDK_ENV}`,
);

// Get the Notification Queue URL from the notification-service CloudFormation output
const notificationQueueUrl = cdk.Fn.importValue(
  `smp-queue-leveling-notification-service-notification-queue-url-${config.CDK_ENV}`,
);
...
new LambdaStack(app, `${config.CDK_APP_NAME}-lambda-stack-${config.CDK_ENV}`, {
  ...
  notificationQueueArn: notificationQueueArn,
