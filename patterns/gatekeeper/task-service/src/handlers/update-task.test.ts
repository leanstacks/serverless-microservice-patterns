...
describe('update-task handler', () => {
  ...
  describe('handler', () => {
    it('should update a task and return 200 with updated task', async () => {
      ...
      expect(mockLoggerInfo).toHaveBeenCalledWith('[UpdateTaskHandler] > handler');
    ...
    it('should return 404 when task is not found', async () => {
      ...
      expect(mockUpdateTask).toHaveBeenCalledTimes(1);
      expect(mockLoggerInfo).toHaveBeenCalledWith({ taskId }, '[UpdateTaskHandler] < handler - task not found');
    });
    ...
    it('should return 400 when title is missing', async () => {
      ...
      expect(mockLoggerWarn).toHaveBeenCalledWith(
        expect.objectContaining({ validationMessages: expect.any(String) }),
