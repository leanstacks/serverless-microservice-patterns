...
export const handler: APIGatewayTokenAuthorizerHandler = async (
  ...
  logger.info('[TokenAuthorizerHandler] > handler');
  logger.debug({ event }, '[TokenAuthorizerHandler] - event');

  try {
    const token = event.authorizationToken;

    if (!token) {
      logger.warn('[TokenAuthorizerHandler] < handler - No authorization token provided');
    ...
    if (!validationResult.isValid) {
      logger.warn({ error: validationResult.error }, '[TokenAuthorizerHandler] < handler - Token validation failed');
    ...
    logger.debug({ baseResource }, '[TokenAuthorizerHandler] - Base resource extracted');

    const policy = buildPolicy(validationResult.principalId!, 'Allow', baseResource, validationResult.context);

    logger.info({ policy }, '[TokenAuthorizerHandler] < handler - authorization allowed');
    return policy;
  } catch (error) {
    logger.error({ error }, '[TokenAuthorizerHandler] - error');

    // Return explicit deny policy
    const methodArn = event.methodArn;
    const policy = buildPolicy('user', 'Deny', methodArn);

    logger.info({ policy }, '[TokenAuthorizerHandler] < handler - authorization denied');
    return policy;
  }
};
